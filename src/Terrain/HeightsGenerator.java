...
public class HeightsGenerator {
    ...
    public float genereteHeight(int x, int z) {
        float total = getInterpolatedNoise(x / 8.0f, z / 8.0f) * AMPLITUDE;
        total += getInterpolatedNoise(x / 4.0f, z / 4.0f) * AMPLITUDE / 3f;
        total += getInterpolatedNoise(x / 2.0f, z / 2.0f) * AMPLITUDE / 3f;
        total += getInterpolatedNoise(x, z) * AMPLITUDE / 3f;
    ...
    private float getNoise(int x, int z) {
        random.setSeed(x * 465234 + z * 345626 + seed);
